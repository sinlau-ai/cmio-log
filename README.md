...
6. **早上 portal :5100 5 小時 outage 全程診斷 + watchdog 自癒功能** — Portal 03:31 crash 後重啟連續 MODULE_NOT_FOUND，watchdog trip circuit breaker 鎖死 5 小時直到 08:41 手動 `npm install` 補回 34 個被秘密刪除的 transitive deps（含 `@mapbox/node-pre-gyp`）。根因高度懷疑 Trend Micro EDR 靜默 quarantine — 跟之前 `claude.exe` 被 path-based 封鎖且不留 Windows log 是同一 pattern。產出：(a) zh-tw HTML 根因報告 `D:\CC\reports\portal-outage-2026-04-27.html` 含 8 個候選假設與時間軸、(b) slh-servers-ops PR [#23](https://github.com/sinlau-ai/slh-servers-ops/pull/23) 為 watchdog 加 self-heal — trip circuit 前先掃 log 找 MODULE_NOT_FOUND 然後 async `npm install`，1h cooldown 防 thrash，下次同樣事件 1 個 tick 內就會自動恢復不用人工介入
