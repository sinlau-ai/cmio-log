...
6. **早上 portal :5100 5 小時 outage 全程診斷 + watchdog 自癒功能** — Portal 03:31 crash 後重啟連續 MODULE_NOT_FOUND，watchdog trip circuit breaker 鎖死 5 小時直到 08:41 手動 `npm install` 補回 34 個被秘密刪除的 transitive deps（含 `@mapbox/node-pre-gyp`）。根因高度懷疑 Trend Micro EDR 靜默 quarantine — 跟之前 `claude.exe` 被 path-based 封鎖且不留 Windows log 是同一 pattern。中央 console 後續查無紀錄、再查 ops watchdog log 後發現 **3:00 nightly-deploy 以 dirty tree 早早 abort 了，沒做 npm ci**；3:31 portal + clinical-llm 同時 health probe failed 更像是 host-level 短暫故障觸發了重啟，但 portal 重啟後 fresh require 才暴露出 @mapbox 早就消失（時點不可考）。產出：(a) zh-tw HTML 根因報告 `D:\CC\reports\portal-outage-2026-04-27.html` 含 8 個候選假設與時間軸、(b) slh-servers-ops PR [#23](https://github.com/sinlau-ai/slh-servers-ops/pull/23) 為 watchdog 加 self-heal — trip circuit 前先掃 log 找 MODULE_NOT_FOUND 然後 async `npm install`，1h cooldown 防 thrash，下次同樣事件 1 個 tick 內就會自動恢復不用人工介入、(c) `D:\CC\reports\trend-micro-exclusion-request.{html,pdf}` IT 工單表單，列出 5 個 clinical service 的 native binary 路徑請 IT 加進 TM exclusion 清單
7. **inpatient PR [#19](https://github.com/sinlau-ai/inpatient/pull/19) MERGED — workspace crash on 出院病人** — feedback fb-1776650004128 寫的「WebPACS: no data when opening」其實不是 WebPACS 問題，而是整個 `/workspace/[mrn]` 頁對已出院病人 server-side render crash（`TypeError: Cannot read properties of undefined (reading 'admit_date') at PatientBanner`）。`/api/patient/summary` 出院病人不回 `info.admission`，但 `lib/types.ts` 把 `admission` 寫成 required，PatientBanner.tsx 直接 deref 沒有 guard。修法：types `admission?` 改 optional + Banner 三段（床號/入院日/主治）包 `{adm && (...)}`，出院病人改顯示「已出院（檢視歷史資料）」badge。E2E 用 agent-browser 登入 d4303 重現後再驗證 fix 通過。dr=4303 自己的病人 MRN 20790781 (王O傑) 04-22 出院，是觸發此 bug 的具體個案
...
### portal :5100 outage 根因分析 + watchdog self-heal (slh-servers-ops PR #23)

清晨來上班發現 portal 已經死了 5 個多小時。最後一次健康啟動是 04-26 23:23，跑了 4 小時後 04-27 03:31:28 以 exit code 1 結束 — 之後每次重啟都在 Node 模組載入階段炸 `Cannot find module '@mapbox/node-pre-gyp'`，watchdog 連 trip circuit 後就鎖在 10 分鐘 backoff 不再嘗試。手動 `npm install` 補回 **34 個 packages**（不只那一個 module，是一整個 transitive subtree）後 portal :5100 立刻恢復、health check 全綠。

根因調查走得比修復本身久。Forensics：(a) `node_modules/` 目錄 mtime 精確指向 03:28；(b) `package.json`、`package-lock.json` 在那個時段都沒被改，**排除任何 npm 主動操作**；(c) 受影響的全是 native-binary 相依（`node-pre-gyp` 是會下載並執行 `.node` 檔的工具）；(d) 一次刪一整個 subtree 不是隨機腐蝕。最強嫌疑：**Trend Micro EDR 在 01:17 pattern 更新後啟動的 real-time scan 把 odbc native binary 隔離掉**，跟之前它把 `claude.exe` 以 path 為依據封鎖、且本機 log 完全空白是同一 pattern。本機 TM logs 全掃過 — Conn log 在 01:19-04:01 有 2.5 小時 audit 缺口剛好涵蓋事件、ofcdebug.log 04:19 才開始記錄、AMSP quarantine 目錄是空的、Application event log 在 03:25-03:35 沒任何 TM provider 事件。完全符合 `CLAUDE.workstation.md` Pitfall 19 描述的「TM 把封鎖決策只寫到中央 console」行為。產出 zh-tw HTML 根因報告 `D:\CC\reports\portal-outage-2026-04-27.html`，含時間軸、9 列關鍵證據表、8 個候選假設（從高機率到極低機率）、以及給 IT 的中央 console 查詢方向。

修復是手動 `npm install`，但李醫師的問題很到位：「為什麼 watchdog 沒自動補上？」 → 立刻在 `slh-servers-ops` 加了 self-heal 階段。新 module `src/watchdog/diagnose.ts` 提供 `detectMissingModule(logFile)` 掃 log tail 抓 `Cannot find module 'X'` / `MODULE_NOT_FOUND` pattern、`runNpmInstall(repoPath)` async spawn `npm install --no-audit --no-fund`（5 分鐘 timeout）。`loop.ts` 在 `handleDown` 即將 `tripCircuit` 之前先呼叫 `maybeKickoffSelfHeal` — 若 enabled、`repoPath` 存在、cooldown 過了，就把 service 標 `healing` 並 async kick off install，本 tick 的 trip 與後續 restart 全部 defer 直到 install 完成。完成後清掉 `healing` flag、重置 `restartHistory`，讓下個 tick 從乾淨 budget 重啟。1 小時 per-service cooldown (`HEAL_COOLDOWN_MS`) 防止 EDR 反覆 quarantine 造成 install loop；`WATCHDOG_SELF_HEAL_ENABLED=false` 可關閉但預設 ON（idempotent npm install 風險很低）。新 `SignalKind` `self_heal_attempted` 寫進 signals 表供 audit 與後續 digest 顯示。

PR [#23](https://github.com/sinlau-ai/slh-servers-ops/pull/23) — 5 files / +330/-2，typecheck 乾淨、133 tests 全綠（含 7 個新 unit test）。已重啟 ops agent、新程式碼上線。下次 portal 又被 EDR 偷檔，1 個 tick (30 秒) 內就會自癒，不用人介入。

過程中也發現一個附帶警訊：`2am ops` 排程任務指向不存在的 `D:\CC\overnight-2am.bat` — 無關本次事件但本身是個 silent failure，列在報告 recommended actions 待後續處理。

<details>
<summary>技術細節</summary>

- agent-portal 修復：`cd D:/repos/agent-portal && npm install` 補回 `@mapbox/node-pre-gyp`、`@npmcli/fs`、`bin-links`、`cacache`、`cross-spawn`、`debug`、`gauge`、`hosted-git-info`、`http-cache-semantics`、`http-proxy-agent`、`https-proxy-agent`、`ip-address`、`is-fullwidth-code-point`、`make-fetch-happen` 等 34 個 packages（npm output 顯示 "added 34 packages, changed 13 packages"）
- 證據時間軸：`stat node_modules → 2026-04-27 03:28:xx`、`stat package-lock.json → 2026-04-15 21:37`、`stat package.json → 2026-04-22 14:13`
- TM 在地 log 全掃：`C:/Program Files (x86)/Trend Micro/Security Agent/{ConnLog,Misc,FLog,HLog,AU_Data}` + `C:/ProgramData/Trend Micro/AMSP/quarantine` + Windows Application Event Log，03:25-03:35 完全沒任何 detection / quarantine 紀錄
- Local Conn log 顯示 01:17:53 Scheduled Update started、01:18:54 RealtimeScan re-enabled、01:19:39 Update Normal End；接下來 2.5 小時靜默到 04:01:29 才有下次 Update — 整個事件視窗在這個 audit 缺口裡
- watchdog new code: `src/watchdog/diagnose.ts`（121 行）、`src/watchdog/loop.ts`（+93 行 self-heal logic + signals + state tracking）、`src/config.ts`（+3 行 `watchdogSelfHealEnabled` flag）、`src/main.ts`（+5 行傳 flag 進 Watchdog ctor）、`tests/diagnose.test.ts`（106 行 / 7 tests：empty file、no pattern、@mapbox extraction、multi-hit returns latest、large-file tail-only scan、stale-above-tail ignored、ENOENT path）
- circuit-trip 前 hook 點：`handleDown` 中 `restartCount >= CIRCUIT_BREAK_COUNT` 但還沒呼叫 `tripCircuit` — 若 `maybeKickoffSelfHeal` 回 true 就 `return` 跳過 trip。`healing: Set<string>` 在 handleDown 第一行檢查、healing 服務直接 skip。`lastHealAttempt: Map<string, number>` 完成時更新（`finally` block 確保異常也記錄）
- ops agent 重啟方法：`Stop-Process -Id <pid> -Force` 殺舊 PID、`Start-Process -FilePath D:\repos\slh-servers-ops\scripts\start-slh-servers-ops.bat -WindowStyle Hidden`。注意 git bash 的 `taskkill /F /PID` 會被 msys 翻譯成 `'F:/'` 路徑 silent fail（已記錄在傳統 traps）
- HTML 報告：offline-first（無 CDN、無外部字型、無 emoji）、ASCII units、CRLF line endings；包含時間軸、關鍵證據 9 列表格、8 個 candidate 章節（每個含支持/不利證據兩欄與 confirm 步驟）、verdict block、recommended actions
